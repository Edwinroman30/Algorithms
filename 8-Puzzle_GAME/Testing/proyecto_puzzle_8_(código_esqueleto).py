...
def bfs_search(initial_state):
    ...
    explored = list() #Replace the set by a list
    ...
    while ( not frontier.isEmpty() ):
        ...
        if(test_goal(node.config)):
            print("Sucess Node Found: " + str(node.config))
            print(f"Node Expanded: {node_expanded}")
...
def main():
    ...
    if sm == "bfs":
        #In this line, we are trying to find the path.
        target, explored = bfs_search(hard_state)
        searchInOrder = []

        while target.parent is not None:
            searchInOrder.append(target.action);
            parent_idx = explored.index(target.parent)
            target = explored[ parent_idx ]
